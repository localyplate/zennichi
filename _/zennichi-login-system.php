...
class zennnichi_login_system {
    ...
    public function __construct() {
        ...
        //
        add_filter('manage_users_columns', array($this, 'rd_user_id_column'));      /* Khang Add for test */
        add_action('manage_users_custom_column', array($this, 'rd_user_id_column_content'), 10, 3);     /* Khang Add for test */
    }
    ...
    public function my_user_meta($contactmethods) {
        ...
        $contactmethods['user_takuchi_office_pref'] = '氏名';
        $contactmethods['user_takuchi_addr'] = '氏名';
        $contactmethods['user_takuchi_tel'] = '氏名';
    ...
    public function generate_excel_redirect() {
        ...
        if (isset($wp_query->query['pagename'])) {
            if ($wp_query->query['pagename'] == "generate-excel") {
                ...
                //
                $xls_template = dirname(__FILE__) . "/tmp/template.xlsx";
                generate_excel(1, $xls_template);
                //
                exit();
            }
        }
    ...
    public function rd_user_id_column($columns) {
        $columns['export'] = 'Export Excel';
        return $columns;
    }
    ...
    public function rd_user_id_column_content($value, $column_name, $user_id) {
...
/* Khang Add for test */
//require_once 'include/generate_excel.php';    /* test */
//$xls_template = dirname(__FILE__) . "/tmp/template.xlsx";
//generate_excel(1, $xls_template);
