...
class zennnichi_login_system{
    ...
    public function tml_user_register( $user_id ) {
        ...
        if ( !empty($_POST['user_region'])) update_user_meta( $user_id, 'user_region', $_POST['user_region']);
        if ( !empty($_POST['user_trade_name'])) update_user_meta( $user_id, 'user_trade_name', $_POST['user_trade_name']);
        if ( !empty($_POST['user_license_holder'])) update_user_meta( $user_id, 'user_license_holder', $_POST['user_license_holder']);
        if ( !empty($_POST['user_license_hokkaido'])) update_user_meta( $user_id, 'user_license_hokkaido', $_POST['user_license_hokkaido']);
        if ( !empty($_POST['user_license_number'])) update_user_meta( $user_id, 'user_license_number', $_POST['user_license_number']);
        //if ( !empty($_POST['user_update_count'])) update_user_meta( $user_id, 'user_update_count', $_POST['user_update_count']);
        if ( !empty($_POST['user_zip'])) update_user_meta( $user_id, 'user_zip', $_POST['user_zip']);
        if ( !empty($_POST['user_addr1'])) update_user_meta( $user_id, 'user_addr1', $_POST['user_addr1']);
        if ( !empty($_POST['user_addr2'])) update_user_meta( $user_id, 'user_addr2', $_POST['user_addr2']);
        if ( !empty($_POST['user_trade_name_kana'])) update_user_meta( $user_id, 'user_trade_name_kana', $_POST['user_trade_name_kana']);
        if ( !empty($_POST['user_user_license_pref'])) update_user_meta( $user_id, 'user_license_pref', $_POST['user_license_pref']);
        if ( !empty($_POST['user_branch_code'])) update_user_meta( $user_id, 'user_branch_code', $_POST['user_branch_code']);
        if ( !empty($_POST['user_representative'])) update_user_meta( $user_id, 'user_representative', $_POST['user_representative']);
        if ( !empty($_POST['user_minor_office'])) update_user_meta( $user_id, 'user_minor_office', $_POST['user_minor_office']);
        
        if ( !empty($_POST['user_license_year'])) update_user_meta( $user_id, 'user_license_year', $_POST['user_license_year']);
        if ( !empty($_POST['user_license_month'])) update_user_meta( $user_id, 'user_license_month', $_POST['user_license_month']);
        if ( !empty($_POST['user_license_day'])) update_user_meta( $user_id, 'user_license_day', $_POST['user_license_day']);
        
        if ( !empty($_POST['user_representative_title'])) update_user_meta( $user_id, 'user_representative_title', $_POST['user_representative_title']);
        if ( !empty($_POST['user_takuchi_pref'])) update_user_meta( $user_id, 'user_takuchi_pref', $_POST['user_takuchi_pref']);
        if ( !empty($_POST['user_takuchi_number'])) update_user_meta( $user_id, 'user_takuchi_number', $_POST['user_takuchi_number']);
        if ( !empty($_POST['user_takuchi_name'])) update_user_meta( $user_id, 'user_takuchi_name', $_POST['user_takuchi_name']);
        if ( !empty($_POST['user_office_pref'])) update_user_meta( $user_id, 'user_office_pref', $_POST['user_office_pref']);
    }
    ...
    public function redirect_exclusion(){
        if($_SERVER['REQUEST_URI'] == '/your-profile/') return false;
        if($_SERVER['REQUEST_URI'] == '/login/') return false;
        if($_SERVER['REQUEST_URI'] == '/logout/') return false;
        if($_SERVER['REQUEST_URI'] == '/resetpass/') return false;
        if($_SERVER['REQUEST_URI'] == '/reset-password/') return false;
        return true;
    }
    ...
    //EXCELエクスポートへのリダイレクト
    public function generate_excel_redirect(){
        global $wp_query;
        if(isset($wp_query->query['pagename'])){
            if($wp_query->query['pagename'] == "generate-excel"){
                header("Content-Type: text/html; charset=UTF-8");
                require 'include/generate_excel.php';
                exit();
            }
        }
    }
