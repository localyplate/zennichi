...
//    $objPHPExcel->setActiveSheetIndex(0);
    ...
    $xlsName = date('Y年m月d日-H時i分s秒') . '-' . $template_id . '-' . $user_id . '.xlsx';
...
function generate_excel_2($template_id = '', $user_id = 0) {
    ...
    foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
        ...
        $highestRow = 60; // e.g. 10
        $highestColumn = 'AI'; // e.g 'F'
        $highestColumnIndex = PHPExcel_Cell::columnIndexFromString($highestColumn);
        ...
        for ($row = 1; $row <= $highestRow; ++$row) {
            for ($col = 0; $col < $highestColumnIndex; ++$col) {
                ...
                //
                unset($cell, $val, $cell_name);
            }
        }
        //
        unset($worksheet, $highestRow, $highestColumn, $highestColumnIndex, $row, $col);
    }

    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8');
    header('Content-Disposition: attachment;filename="' . $template_id . '"');
    header("Cache-Control: max-age=0");
    header("Pragma: public");
    header("Expires: 0");
    ...
    // free memory
    unset($template, $user_id, $data);
    unset($objPHPExcel);
    unset($objWriter);
    exit();
}

function generate_excel_3($template_id = '', $user_id = 0) {
    ...
    foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
        ...
        foreach ($worksheet->toArray(null, true, true, true) as $row => $col) {
...
//print_r('1.' . convert(memory_get_usage(true)) . '<br>');
